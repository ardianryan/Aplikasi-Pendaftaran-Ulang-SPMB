...
export async function uploadDocument(c: Context) {
  ...
  const docType = c.req.param("docType") as DocumentType;

  // Validate document type
  if (!DOCUMENT_TYPES.includes(docType)) {
    return error(
      c,
      `Tipe dokumen tidak valid. Pilihan: ${DOCUMENT_TYPES.join(", ")}`,
      400
    );
  }
  ...
  try {
    ...
    if (student?.dokumen?.[docType]?.key) {
      try {
        await deleteFromR2(student.dokumen[docType]!.key);
      } catch (deleteErr) {
        // Non-critical: old file deletion failed, continue with upload
        console.warn(`[UPLOAD] Failed to delete old file: ${student.dokumen[docType]!.key}`);
      }
    }

    // Upload to R2
    const result = await uploadToR2(nisn, docType, buffer, file.type, file.name);
...
export async function deleteDocument(c: Context) {
  ...
  try {
    ...
    const docInfo = student.dokumen?.[docType];
