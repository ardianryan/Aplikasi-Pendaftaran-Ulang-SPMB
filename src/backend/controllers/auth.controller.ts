 ...
 */
async function verifyGoogleAccessToken(credential: string, _email?: string): Promise<{
  email: string;
  name: string;
  picture: string;
} | null> {
  // Always use ID Token verification for GSI (Google Identity Services) flow
  // as it returns a JWT ID token.
...
export async function activateOperator(c: Context) {
  try {
    ...
    // Use SSO name if provided, otherwise fallback to Google name
    const finalName = ssoName || googleUser.name || name || "Operator";
    ...
    // Create operator account with SSO info
